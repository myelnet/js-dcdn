...
import type {ContentRoutingInterface, DealOffer} from '@dcdn/routing';
...
import type {PaychMgr} from './paychmgr';
...
export class DataTransfer extends EventEmitter {
  ...
  _receiveMessage = (data: Uint8Array) => {
    ...
    if (!res.Acpt) {
      ...
      // TODO
      ch.send({type: 'DEAL_REJECTED', error: err});
    }
  };
      ...
      load: async (blk: CID): Promise<Block<any>> => {
        ...
        if (!request) {
          ...
          try {
            ...
            this.network.peerStore.addressBook.add(offer.id, offer.multiaddrs);
