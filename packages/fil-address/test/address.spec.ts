...
import {newSecp256k1Address, newFromString, newIDAddress} from '../src/address';
import {equals} from 'uint8arrays/equals';

describe('filaddress', () => {
  it('it should create new ID addresses', async () => {
    IDAddresses.forEach((item) => {
      const address = newIDAddress(item.string.slice(2));
      expect(equals(Uint8Array.from(address.str), item.decodedByteArray)).to.be
        .true;
    });
  });
  describe('newSecp256k1Address', () => {
    it('it should create new Secp256k1 address', () => {
      const encoder = new TextEncoder();
      const data = encoder.encode('Secp256k1 pubkey');
      const address = newSecp256k1Address(data);
      expect(equals(address.payload(), blake2b(data, undefined, 20))).to.be
        .true;
    });

    it('it should create new secp256k1 addresses from string', async () => {
      secp256k1Addresses.forEach((item) => {
        const address = newFromString(item.string);
        expect(equals(Uint8Array.from(address.str), item.decodedByteArray)).to
          .be.true;
      });
    });
  });

  it('it should create new Actor addresses', async () => {
